...
func TestRunCommandsInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		stdout, err := captureOutput(t, func(r *runner) error {
			return r.run([]string{"get", "51", "--path"})
		})
		if err == nil {
		...
		stdout, err = captureOutput(t, func(r *runner) error {
			return r.run([]string{"get", "51", "--json"})
		})
		if err == nil {
		...
		if err := newTestRunner().run([]string{"open", "51"}); err != nil {
		...
		if err := newTestRunner().run([]string{"close", "51"}); err != nil {
...
func TestRunCommandErrorPathsInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		if err := newTestRunner().run([]string{"close", "missing"}); err != nil {
...
func TestRunStructuredOutputRequiresMetadataForExistingWorktree(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		...
		if err := newTestRunner().run([]string{"get", "51", "--json"}); err == nil {
			t.Fatal("get --json with missing metadata error = nil, want error")
		}
		if err := newTestRunner().run([]string{"get", "51", "--toon"}); err == nil {
...
func TestRunGetAllModesInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		...
		branch, err := captureOutput(t, func(r *runner) error { return r.run([]string{"get", "51", "--branch"}) })
		if err != nil {
		...
		root, err := captureOutput(t, func(r *runner) error { return r.run([]string{"get", "51", "--root"}) })
		if err != nil {
		...
		toon, err := captureOutput(t, func(r *runner) error { return r.run([]string{"get", "51", "--toon"}) })
		if err != nil {
		...
		shortJSON, err := captureOutput(t, func(r *runner) error { return r.run([]string{"get", "51", "--json"}) })
		if err != nil {
			t.Fatalf("get --json error = %v", err)
		}
		formatJSON, err := captureOutput(t, func(r *runner) error { return r.run([]string{"get", "51", "--format", "json"}) })
		if err != nil {
