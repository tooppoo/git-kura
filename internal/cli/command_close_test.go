...
func TestRunCloseErrorPathsInProcess(t *testing.T) {
	...
	t.Run("dirty worktree", func(t *testing.T) {
		...
		withWorkingDir(t, repo, func() {
			if err := newTestRunner().run([]string{"open", "52"}); err != nil {
				t.Fatalf("open error = %v", err)
			}
			appendFile(t, filepath.Join(expectedWorktreePath(repo, "52"), "tracked.txt"), "dirty\n")
			if err := newTestRunner().run([]string{"close", "52"}); err == nil {
	...
	t.Run("unmerged branch", func(t *testing.T) {
		...
		repo := cli.initRepo(t)
		withWorkingDir(t, repo, func() {
			if err := newTestRunner().run([]string{"open", "53"}); err != nil {
				t.Fatalf("open error = %v", err)
			}
			...
			if err := newTestRunner().run([]string{"close", "53"}); err == nil {
...
func TestRunCloseLockTimeoutInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		...
		err = newTestRunner().run([]string{"close", "51"})
		if err == nil {
			t.Fatal("close with held lock error = nil, want seal-lock-timeout")
		}
		var xe *exitError
		if !errors.As(err, &xe) || xe.code != int(exitSealLockTimeout) {
