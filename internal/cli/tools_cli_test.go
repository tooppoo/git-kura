...
func TestToolsHelp(t *testing.T) {
	...
	for _, args := range [][]string{{"-h"}, {"--help"}, {"status", "-h"}, {"install", "--help"}, {"uninstall", "-h"}} {
		out, err := captureOutput(t, func(r *runner) error { return r.runToolsWith(deps, args) })
		if err != nil {
	...
	if _, err := captureOutput(t, func(r *runner) error { return r.runTools([]string{"--help"}) }); err != nil {
...
func TestToolsFailsOutsideGitRepository(t *testing.T) {
	...
	withWorkingDir(t, dir, func() {
		_, err = captureOutput(t, func(r *runner) error { return r.runToolsWith(deps, []string{"status"}) })
	})
	if err == nil {
...
func TestToolsRunDispatch(t *testing.T) {
	if err := newTestRunner().runToolsRun([]string{"--help"}); err != nil {
		t.Fatalf("help: %v", err)
	}
	if got := exitCodeOf(newTestRunner().runToolsRun(nil)); got != exitUsageError {
		t.Fatalf("no args should be usage error, got %d", got)
	}
	if got := exitCodeOf(newTestRunner().runToolsRun([]string{"bogus"})); got != exitUsageError {
...
func TestRunToolsWithRunSubcommand(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		_, err = captureOutput(t, func(r *runner) error { return r.runToolsWith(deps, []string{"run", "pre-commit"}) })
	})
	if err != nil {
