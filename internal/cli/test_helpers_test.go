...
func newTestRunner() *runner {
	return &runner{stdout: io.Discard, stderr: io.Discard, version: testVersion}
}

func captureOutput(t *testing.T, fn func(*runner) error) (string, error) {
	t.Helper()
	var buf bytes.Buffer
	r := &runner{stdout: &buf, stderr: io.Discard, version: testVersion}
	err := fn(r)
...
func newTestCLI(t *testing.T) *testCLI {
	...
	build := exec.Command("go", "build", "-o", bin, "github.com/tooppoo/git-kura/cmd/git-kura")
...
func openManagedWorktree(t *testing.T, repo, key string) string {
	...
	withWorkingDir(t, repo, func() {
		out, err := captureOutput(t, func(r *runner) error { return r.cmdOpen(key, openOptions{}) })
		if err != nil {
