...
func TestRenderedErrorErrorIsEmpty(t *testing.T) {
	...
	if got := (&renderedError{code: int(exitUnsafeRefused)}).Error(); got != "" {
...
func TestExitErrorUnwrap(t *testing.T) {
	...
	xe := &exitError{code: int(exitNotFound), err: inner}
...
func TestReasonForExitCode(t *testing.T) {
	cases := map[int]string{
		int(exitUsageError):      "usage-error",
		int(exitUnsafeRefused):   "unsafe-refused",
		int(exitNotFound):        "not-found",
		int(exitSealLockTimeout): "seal-lock-timeout",
		int(exitSealConflict):    "seal-conflict",
		int(exitSealDoctorError): "seal-doctor-error",
		int(exitGeneralError):    "general-error",
		9999:                     "general-error",
	}
...
func TestToCommandError(t *testing.T) {
	t.Run("plain error is a general error", func(t *testing.T) {
		...
		if ce.Code != "general-error" || ce.ExitCode != int(exitGeneralError) {
	...
	t.Run("exitError keeps its code and reason", func(t *testing.T) {
		...
		if ce.Code != "not-found" || ce.ExitCode != int(exitNotFound) {
...
func TestCmdSealDoctorOutsideRepoFails(t *testing.T) {
	...
	withWorkingDir(t, outside, func() {
		if err := newTestRunner().cmdSealDoctor(sealDoctorOptions{}); err == nil {
...
func TestCmdSealDoctorReturnsExitCode7ForIntegrityFailures(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		out, err := captureOutput(t, func(r *runner) error {
			return r.cmdSealDoctor(sealDoctorOptions{})
		})
		if err == nil {
			t.Fatal("cmdSealDoctor invalid store error = nil, want error")
		}
		var re *renderedError
		if !errors.As(err, &re) || re.code != int(exitSealDoctorError) {
...
func TestExitCodeErrorNilPassthrough(t *testing.T) {
	...
	var xe *exitError
	if !errors.As(err, &xe) || xe.code != int(exitSealConflict) {
...
func TestCmdSealClaimAndRemoveInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"tracked.txt"}); err != nil {
		...
		if err := newTestRunner().cmdSealUnclaim(sealUnclaimOptions{}, []string{"tracked.txt"}); err != nil {
...
func TestCmdSealClaimMultiplePathsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt1, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"tracked.txt", "second.txt", "third.txt"}); err != nil {
			t.Fatalf("cmdSealClaim multiple paths: %v", err)
		}
	})
	// All three should be blocked for a different key
	withWorkingDir(t, wt2, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"second.txt"}); err == nil {
...
func TestCmdSealClaimRejectsDifferentKeyInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt2, func() {
		err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"tracked.txt"})
		if err == nil {
...
func TestCmdSealUnclaimRejectsDifferentKeyInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt2, func() {
		err := newTestRunner().cmdSealUnclaim(sealUnclaimOptions{}, []string{"tracked.txt"})
		if err == nil {
...
func TestCmdSealClaimReportsAllConflictsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt3, func() {
		err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"tracked.txt", "second.txt", "third.txt"})
		if err == nil {
			t.Fatal("expected conflict error, got nil")
		}
		msg := err.Error()
	...
	withWorkingDir(t, wt4, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"third.txt"}); err != nil {
...
func TestCmdSealClaimRejectsDirectoryInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"subdir"})
		if err == nil {
...
func TestCmdSealClaimNonExistentFileInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"nosuchfile.txt"}); err == nil {
...
func TestCmdSealClaimOutsideRepoInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"../outside.txt"}); err == nil {
			t.Fatal("expected error for path outside repo, got nil")
		}
	})
}
...
func TestCmdSealUnclaimOutsideRepoInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealUnclaim(sealUnclaimOptions{}, []string{"../outside.txt"}); err == nil {
			t.Fatal("expected error for path outside repo, got nil")
		}
	})
}
...
func TestCmdSealUnclaimFromMultiPathStoreInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt1, func() {
		if err := newTestRunner().cmdSealClaim(sealClaimOptions{}, []string{"tracked.txt", "second.txt"}); err != nil {
			t.Fatalf("cmdSealClaim: %v", err)
		}
		if err := newTestRunner().cmdSealUnclaim(sealUnclaimOptions{}, []string{"tracked.txt"}); err != nil {
...
func TestRunSealClaimUnclaimInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().run([]string{"seal", "claim", "tracked.txt"}); err != nil {
			t.Fatalf("seal claim via run: %v", err)
		}
		if err := newTestRunner().run([]string{"seal", "unclaim", "tracked.txt"}); err != nil {
...
func TestRunSealClaimMultiplePathsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().run([]string{"seal", "claim", "tracked.txt", "second.txt"}); err != nil {
			t.Fatalf("seal claim multiple paths via run: %v", err)
		}
		if err := newTestRunner().run([]string{"seal", "unclaim", "tracked.txt", "second.txt"}); err != nil {
...
func TestRunSealClaimMissingArgInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		if err := newTestRunner().run([]string{"seal", "claim"}); err == nil {
			t.Fatal("expected error for missing path arg, got nil")
		}
		if err := newTestRunner().run([]string{"seal", "unclaim"}); err == nil {
			t.Fatal("expected error for missing path arg, got nil")
		}
	})
}
...
func TestRunSealClaimUnclaimHelpInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		stdout, err := captureOutput(t, func(r *runner) error {
			return r.run([]string{"seal", "claim", "--help"})
		})
		if err != nil {
		...
		stdout, err = captureOutput(t, func(r *runner) error {
			return r.run([]string{"seal", "unclaim", "--help"})
		})
		if err != nil {
...
func TestCmdSealClaimJSONSuccessInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		out, err := captureOutput(t, func(r *runner) error {
			return r.cmdSealClaim(sealClaimOptions{JSON: true}, []string{"tracked.txt"})
		})
		if err != nil {
...
func TestCmdSealUnclaimJSONSuccessInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		...
		out, err := captureOutput(t, func(r *runner) error {
			return r.cmdSealUnclaim(sealUnclaimOptions{JSON: true}, []string{"tracked.txt"})
		})
		if err != nil {
...
func TestCmdSealTestUnsealedSucceedsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealTest(sealTestOptions{}, []string{"tracked.txt"}); err != nil {
...
func TestCmdSealTestNonExistentPathSucceedsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealTest(sealTestOptions{}, []string{"new-file.txt"}); err != nil {
...
func TestCmdSealTestRejectsDifferentKeyInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt2, func() {
		out, err := captureOutput(t, func(r *runner) error {
			return r.cmdSealTest(sealTestOptions{}, []string{"tracked.txt"})
		})
		if err == nil {
			t.Fatal("expected conflict error for path claimed by different key, got nil")
		}
		var re *renderedError
		if !errors.As(err, &re) || re.code != int(exitSealConflict) {
...
func TestCmdSealTestReportsAllConflictsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt3, func() {
		out, err := captureOutput(t, func(r *runner) error {
			return r.cmdSealTest(sealTestOptions{}, []string{"tracked.txt", "second.txt", "third.txt"})
		})
		if err == nil {
			t.Fatal("expected conflict error, got nil")
		}
...
func TestCmdSealTestOutsideRepoPathInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().cmdSealTest(sealTestOptions{}, []string{"../outside.txt"}); err == nil {
			t.Fatal("expected error for path outside repo, got nil")
		}
	})
}
...
func TestCmdSealTestFailsOutsideManagedWorktreeInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		err := newTestRunner().cmdSealTest(sealTestOptions{}, []string{"tracked.txt"})
		if err == nil {
			t.Fatal("expected error outside a managed worktree, got nil")
		}
		if strings.Contains(err.Error(), "seal-conflict:") {
...
func TestRunSealTestInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		if err := newTestRunner().run([]string{"seal", "test", "tracked.txt"}); err != nil {
...
func TestRunSealTestMissingArgInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		if err := newTestRunner().run([]string{"seal", "test"}); err == nil {
			t.Fatal("expected error for missing path arg, got nil")
		}
	})
}
...
func TestRunSealTestRejectsUndefinedOptionsInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		for _, opt := range []string{"--all", "--unsealed", "--staged"} {
			if err := newTestRunner().run([]string{"seal", "test", opt, "tracked.txt"}); err == nil {
...
func TestRunSealTestHelpInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		stdout, err := captureOutput(t, func(r *runner) error {
			return r.run([]string{"seal", "test", "--help"})
		})
		if err != nil {
...
func TestRunSealLsJSONInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		var err error
		out, err = captureOutput(t, func(r *runner) error { return r.run([]string{"seal", "ls", "--json"}) })
		if err != nil {
...
func TestRunLsJSONInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		var err error
		out, err = captureOutput(t, func(r *runner) error { return r.run([]string{"ls", "--json"}) })
		if err != nil {
...
func TestSealTestFailJSONEmitsErrorEnvelope(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealTestFail(sealTestOptions{JSON: true}, fmt.Errorf("test-error"))
	})
	if err == nil {
...
func TestSealDoctorFailJSONEmitsErrorEnvelope(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealDoctorFail(sealDoctorOptions{JSON: true}, fmt.Errorf("test-error"))
	})
	if err == nil {
...
func TestCmdSealDoctorJSONHealthyInProcess(t *testing.T) {
	...
	withWorkingDir(t, repo, func() {
		var err error
		out, err = captureOutput(t, func(r *runner) error {
			return r.cmdSealDoctor(sealDoctorOptions{JSON: true})
		})
		if err != nil {
...
func TestCmdSealTestJSONPassedInProcess(t *testing.T) {
	...
	withWorkingDir(t, wt, func() {
		var err error
		out, err = captureOutput(t, func(r *runner) error {
			return r.cmdSealTest(sealTestOptions{JSON: true}, []string{"tracked.txt"})
		})
		if err != nil {
...
func TestCmdSealTestJSONCurrentKeyUnresolvedOutsideGitInProcess(t *testing.T) {
	...
	withWorkingDir(t, outside, func() {
		var err error
		out, err = captureOutput(t, func(r *runner) error {
			return r.cmdSealTest(sealTestOptions{JSON: true}, []string{"any.txt"})
		})
		if err == nil {
			t.Fatal("expected error outside git repo, got nil")
		}
...
func TestCloseFailJSONNilPartial(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.closeFail(closeOptions{JSON: true}, nil, "preflight", fmt.Errorf("no git repo"))
	})
	if err == nil {
...
func TestCloseFailJSONWithPartial(t *testing.T) {
	...
	out, err := captureOutput(t, func(r *runner) error {
		return r.closeFail(closeOptions{JSON: true}, partial, "remove-branch", fmt.Errorf("branch error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealClaimFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealClaimFail(sealClaimOptions{JSON: true}, "preflight", fmt.Errorf("preflight error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealClaimStoreFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealClaimStoreFail(sealClaimOptions{JSON: true}, "read-store", "/path/to/seals.json", fmt.Errorf("read error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealUnclaimFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealUnclaimFail(sealUnclaimOptions{JSON: true}, "preflight", fmt.Errorf("preflight error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealUnclaimStoreFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealUnclaimStoreFail(sealUnclaimOptions{JSON: true}, "write-store", "/path/to/seals.json", fmt.Errorf("write error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestCloseStoreFailJSONWithPartial(t *testing.T) {
	...
	out, err := captureOutput(t, func(r *runner) error {
		return r.closeStoreFail(closeOptions{JSON: true}, partial, "validate-store", "/path/to/paths.json", fmt.Errorf("schema error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealClaimValidateStoreFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealClaimStoreFail(sealClaimOptions{JSON: true}, "validate-store", "/path/to/seals.json", fmt.Errorf("schema error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealUnclaimValidateStoreFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealUnclaimStoreFail(sealUnclaimOptions{JSON: true}, "validate-store", "/path/to/seals.json", fmt.Errorf("schema error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestCloseStoreFailJSON(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.phase, func(t *testing.T) {
			out, err := captureOutput(t, func(r *runner) error {
				return r.closeStoreFail(closeOptions{JSON: true}, nil, tc.phase, "/path/to/paths.json", fmt.Errorf("store error"))
			})
			if err == nil {
				t.Fatal("expected error, got nil")
			}
...
func TestLsFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.lsFail(lsOptions{JSON: true}, fmt.Errorf("ls error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestSealLsFailJSON(t *testing.T) {
	out, err := captureOutput(t, func(r *runner) error {
		return r.sealLsFail(sealLsOptions{JSON: true}, fmt.Errorf("seal ls error"))
	})
	if err == nil {
		t.Fatal("expected error, got nil")
	}
